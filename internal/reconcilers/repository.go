...
func (r *Reconciler) handleRepoReconcilerEvent(msg *message.Message) error {
	...
	ctx := msg.Context()
	log.Printf("handling reconciler event for project %s and repository %+v", evt.Project.String(), evt.Properties)
...
func (r *Reconciler) handleArtifactsReconcilerEvent(ctx context.Context, evt *messages.RepoReconcilerEvent) error {
	lookByProps, err := properties.NewProperties(evt.Properties)
	if err != nil {
		return fmt.Errorf("error creating repository properties: %w", err)
	}

	entRefresh := entityMessage.NewEntityRefreshAndDoMessage().
		WithEntity(pb.Entity_ENTITY_REPOSITORIES, lookByProps).
		WithProviderIDHint(evt.Provider).
		WithProjectIDHint(evt.Project)
	...
	m.SetContext(ctx)
	if err := r.evt.Publish(events.TopicQueueRefreshEntityAndEvaluate, m); err != nil {
		return fmt.Errorf("error publishing message: %w", err)
	}

	// the code below this line will be refactored in a follow-up PR
	repoID, err := lookByProps.GetProperty(properties.PropertyUpstreamID).AsInt64()
	if err != nil {
		return errors.New("repository ID not found in event or not int64")
	}
